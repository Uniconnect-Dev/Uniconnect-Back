...
public class StudentOrgProfileService {
    ...
    public StudentOrgGetResponse initProfile(Long userId, StudentOrgInitRequest request) {
        // 이미 프로필이 있는지 확인
        if (studentOrgRepository.existsByUser_UserId(userId)) {
    ...
    public StudentOrgGetResponse getProfile(Long userId) {
        StudentOrg studentOrg = studentOrgRepository.findByUser_UserId(userId)
    ...
    public StudentOrgGetResponse updateProfile(Long userId, StudentOrgUpdateRequest request) {
        ...
        if (request.getSchoolName() != null) {
            studentOrg.setSchoolName(request.getSchoolName());
        }
        if (request.getOrganizationName() != null) {
            studentOrg.setOrganizationName(request.getOrganizationName());
        }
        if (request.getManagerName() != null) {
            studentOrg.setManagerName(request.getManagerName());
        }
        if (request.getPhone() != null) {
            studentOrg.setPhone(request.getPhone());
        }
        if (request.getEmail() != null) {
            studentOrg.setEmail(request.getEmail());
        }
        if (request.getLogoUrl() != null) {
            studentOrg.setLogoUrl(request.getLogoUrl());
        }
